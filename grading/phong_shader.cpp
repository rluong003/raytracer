    ...
    const vec3& normal,int recursion_depth) const
{
  ...
  //  diff = diff + Rd * Ld * ln;
   ...
   // if shadows arent enabled,, or shhardows are enables,, but theres no oobject intersction or theh object distance is greater than theh length of thhe light to intersection point vector   
...
// if theh shhadow coniditionals are skipped, then the color will only be ambient, if one of thhe shhadow conditionals pass, thhehn diffuse and specular are added or else they are just 0 
	color = amb + diff + spec;
    return color;
}
